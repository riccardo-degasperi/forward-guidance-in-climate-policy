...
dataOpt.shockVar        = {'HICP_EN','EA1YOIS'};          %shock of interest (position in the impact matrix)
...
dataOpt.varendo         = {'HICP_EN','GHG_IP','HICP_ALL','IP','EA1YOIS','UNEMP','STOXX50','ROILEU'};
dataOpt.varendo_global  = {};
dataOpt.varplot         = dataOpt.varendo;
dataOpt.varexo          = {};                   %exogenous variables
dataOpt.units           = {'data'};             %units (can be more than one)
...
dataOpt.srScript        = '';                   %name of script defining sign restrictions (customise it before launch)
dataOpt.mrScript        = '';                   %name of script defining magnitude restrictions (customise it before launch)
dataOpt.variv           = {'F2_CM','jk_MP_median'};
dataOpt.shockIV         = [1 2];                  %position of shocks in shockVar to be identified with proxies
...
miscOpt.SSAirf          = 1;                    %1:on; 2:off
miscOpt.SSAts           = 0;                    %1:on; 2:off
ssaOpt.SSAlT            = '';                   %(for SSAts) sets lower Xlim for scenarios plot
ssaOpt.SSAscript        = 'SSArestrictions';    %name of function defining SSA restrictions (customise it before launch)
ssaOpt.KLtresh          = 0.7;                  %threshold for KL divergence
ssaOpt.impulse          = {'HICP_EN'};          %generate IRFs to shocks in position 'impulse' (you can set this to dataOpt.shockVar)
ssaOpt.SSAy             = {'EA1YOIS'};            %variables whose path is restricted (can define many in this format SSAy = {{'var1','var2'};{'var1'};{'var1','var2','var3'}};)
ssaOpt.SSAe             = {'EA1YOIS'};            %variables whose innovations are used to deliver the path
ssaOpt.customLabels     = {};
...
plotOpt.cb90            = 0;                    %1:plots also 90% confidence bands; 0:plots only 68% confidence bands
plotOpt.folder          = ['figures_4_C6_C7_C8'];  %either a folder name or []
