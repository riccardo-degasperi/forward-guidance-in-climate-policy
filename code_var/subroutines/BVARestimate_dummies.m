...
% vec(B)|psi,Y ~ N(vec(beta),kron(psi,(X'X)^-1)), where X includes dummies
% psi|Y        ~ IW(sigma,T+Td+2-N*p-1)
...
% NOTES:
% X ~ MN(M,U,V)  IFF  vec(X) ~ N(vec(M),kron(V,U))
% Let X ~ MN(0,I,I). Let Y = M + AXB. Then Y ~ MN(M,AA',BB')
% If Z ~ MN(In,Ip) then Z'Z ~ W(Ip,n)
% If Zi ~ N(0,S)   then Z'Z ~ W(S,n)
% Let S = AA'. Then W(S,df) = AW(I,df)A'
