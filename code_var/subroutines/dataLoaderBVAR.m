...
%-------------------------------------------------------------------------%    
    ...
    if i == sIV
        ...
        if strcmp(identification,'iv') || strcmp(identification,'iv+sign')
...
%-------------------------------------------------------------------------%    
    ...
    if i == sTrans
        ...
        % FD index (for first differences)
        iFD_endo = raw.data.(raw.sheets{i})(iTrEn,3);
        ...
        % FD index (for first differences)
        iFD_exo = raw.data.(raw.sheets{i})(iTrEx,3);
        ...
        % FD index (for first differences)
        iFD_global = raw.data.(raw.sheets{i})(iTrEn2,3);
...
% NOTE: multiplying by 100 makes sure the effect of dx on y is beta when y
% is a rate and x is logged.
% If y is logged and x is a rate, the 100 cancels out in the interpretation
% of dx on y.
...
out.iFD_endo              = iFD_endo;
out.iFD_exo               = iFD_exo;
out.iFD_global            = iFD_global;
out.iLogsEn               = iLogsEn;
out.iLogsEn_global        = iLogsEn_global;
out.iLogsEx               = iLogsEx;
if strcmp(modelOpt.identification,'iv') || strcmp(modelOpt.identification,'iv+sign')
    out.iv                    = iv;
    out.ivDates               = ivDates;
end
