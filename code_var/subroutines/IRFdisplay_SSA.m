function [ ] = IRFdisplay_SSA(IRFstruc,varlist,longnames,shockVar,legNames,plotCase,plotOpt,chartName)
...
cbands = [1 .7 .7 1 .4 .4;.7 .7 1 .4 .4 1];
cmedian = [1 0 0;0 0 1];
...
for t = 1:Nfig
    ...
    for i = (t-1)*rows*cols+1:t*rows*cols
        ...
        %for j = 1:numel(names)
            ...
            IRFpatch1=patch(Xpatch1,Ypatch1,cbands(1,1:3));  %build confidence bands 90%
            ...
            IRFpatch1=patch(Xpatch1,Ypatch1,cbands(1,4:6));  %build confidence bands 68%
            set(IRFpatch1,'facealpha',.5);                   %add transparency
            set(IRFpatch1,'edgecolor','none');               %remove edge color

            % Plot median and X-axis
            go(1) = plot(0:h-1,tmp(3,:),style{1},'LineWidth',2,'Color',cmedian(1,:)); %plot median


            % Plot counterfactual median only
            tmp = IRFstruc.(names{2}){i,iShock};
%             tmp0 = [tmp0 tmp];
            go(2) = plot(0:h-1,tmp(3,:),style{2},'LineWidth',2,'Color',cmedian(2,:)); %plot median
        ...
        if cb90
        minband=min(min(tmp0(1,:)),min(tmp(3,:)));        %minimum value of lower bound
        maxband=max(max(tmp0(5,:)),max(tmp(3,:)));        %maximum value of upper bound
        else
        minband=min(min(tmp0(2,:)),min(tmp(3,:)));        %minimum value of lower bound
        maxband=max(max(tmp0(4,:)),max(tmp(3,:)));        %maximum value of upper bound
        end
        ...
        % Create titles
        title(longnames{1,i},'FontWeight','bold');  %variable shocked
