...
% When iv+sign, order the IV shocks first
if strcmp(modelOpt.identification,'iv+sign') || strcmp(modelOpt.identification,'iv')
    if max(dataOpt.shockIV) > numel(dataOpt.shockIV)
        error('ERROR: order the variables in shockVar and the restrictions in SRscript.m so that the proxy identified shocks are ordered first. Check dataOpt.shockIV.')
    end
end
