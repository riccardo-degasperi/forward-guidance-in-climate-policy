...
dataOpt.shockVar        = {'EUA_fq','es600','CMslope_6m1m'}; %shock of interest (position in the impact matrix)
...
dataOpt.varendo         = {'HICP_EN','GHG_IP','HICP_ALL','IP','INT3M','UNEMP','STOXX50','ROILEU','EUA_fq','es600','CMslope_6m1m'};
dataOpt.varendo_global  = {};
dataOpt.varplot         = {'HICP_EN','GHG_IP','HICP_ALL','IP','INT3M','UNEMP','STOXX50','ROILEU'};
dataOpt.varexo          = {};                   %exogenous variables
dataOpt.units           = {'data'};             %units (can be more than one)

% Identification
modelOpt.identification = 'jarocinski-karadi';  %'cholesky', 'iv', 'sign', 'jarocinski-karadi', 'iv+sign'
...
% Options for BVAR
hp.GLP                  = 0;                    %optimal prior selection
...
hp.covid                = PP(z);                %pandemic priors (Cascaldi-Garcia, 2024)
...
plotOpt.whichPlot       = 'varplot1';            %either 'all','varendo','varplot','colour'
plotOpt.cb90            = 0;                    %1:plots also 90% confidence bands; 0:plots only 68% confidence bands
plotOpt.folder          = ['figures_6_C5/PP',num2str(PP(z))];  %either a folder name or []

plotOpt.allSeries.plotRows = 3;                 %max number of rows per plot
