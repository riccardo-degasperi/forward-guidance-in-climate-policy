...
channel = {
    'ESI','EEI',...
    'CF_GDP_1Y','CF_C_1Y','CF_INFL_1Y','CF_UR_1Y',...
    'VSTOXX',...
    'DJSI_STOXX600',...
    'EUI','TRI','PRI','MCCC'
    };
...
sto = cell(size(channel,2));
longnames = cell(1,size(channel,2));
sto_ylab = cell(size(channel,2),1);

for c = 1:CC

clearvars -except c CC channel sto longnames sto_ylab
close all
...
dataOpt.shockVar        = {'HICP_EN'};          %shock of interest (position in the impact matrix)
dataOpt.shockUnit       = {};                   %unit originating the shock (leave empty if policy variable is in sheet "Global")
dataOpt.shockSign       = 'negative';           %either 'positive' or 'negative'
dataOpt.shockSize       = '1sd';                %magnitude of the shocks. Either '1sd' or a scalar (also negative)
dataOpt.varendo         = {'HICP_EN','GHG_IP','HICP_ALL','IP','INT3M','STOXX50','ROILEU',channel{c}};
...
plotOpt.folder          = ['figures_5/',channel{c}];  %either a folder name or []
...
% Store results for channel
sto(c,1) = BVAR_d.IRFsulm(end,1);
longnames(c) = dataOpt.varendolong(end);
sto_ylab(c,1) = plotOpt.ylab(end);

end

% Plot only responses for channels
chartName0 = 'all_channels';
plotOpt.folder = 'figures_5';
plotOpt.ylab = sto_ylab;
plotOpt.allSeries.plotRows = 3;
plotOpt.allSeries.plotCols = 4;
IRFdisplay_single(sto,channel,longnames,channel(1),chartName0,'allSeries',plotOpt)